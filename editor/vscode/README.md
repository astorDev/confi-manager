...
2. CLI either hints or opens that directory. (Can use `code` command if available) 🚧 Pending flow design
3. In VS Code we can edit the config value with schema validation built-in. 🚧 Pending testing of validation
4. In CLI an `apply` / `save` command is called to apply the changes. 🚧 Pending command name decision
